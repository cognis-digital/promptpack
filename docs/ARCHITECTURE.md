...
- **rules/analyzers** apply the heuristics shipped in `promptpack/core.py`.
- **score** ranks by severity.
- **MCP server** (`promptpack mcp`) exposes `scan` for Cognis.Studio agents.
