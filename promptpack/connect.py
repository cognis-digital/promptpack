"""Native cognis-connect emit for promptpack — forward findings to any platform.

Maps promptpack's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    promptpack ... --format json | promptpack-emit --to stix
    promptpack-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop("source", None)
        out["title"] = out.get("title", "No Title")
